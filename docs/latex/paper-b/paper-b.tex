...
\author{Sean D. Egan \and Derek Conklin\\
  \normalsize The Informed --- \texttt{theinformed.org}\\
  \normalsize Corresponding author: \texttt{sean@theinformed.org}}
\date{Preprint, 21 September 2026}
\maketitle
...
A second registration settles it. Phase 3 re-measured the reweighted floor on the \textbf{entire retained archive} --- sampling modulus 1, 89,402,955 passive and 63,018,120 payload usable intervals, five times this paper's sampled passive exposure --- and returned a registered \textbf{FAIL}. The clause this paper failed now passes decisively: the reweighted floor is 0.276 per 1,000 with a clustered object-level bootstrap 95\% upper bound of 0.5025 against the 1.000 target, seed 20260921 over 2,000 draws, where the sampled evidence gave 1.846. The gate is shut by the other clause instead, at a reweighted separation of \textbf{5.36$\times$} against the unchanged 10$\times$ requirement%
...
. That failure is not a power problem and no exposure can buy it off: collapse the passive interval to its point estimate and the separation is 9.77$\times$; read both sides at their point estimates, with no uncertainty admitted anywhere, and it is 9.81$\times$ --- still under the bar%
...
. The audit therefore shut its own gate twice, by two independent routes --- 8.83$\times$ as a matched composite on a 1-in-5 sample, 5.36$\times$ fully reweighted at full-archive exposure --- and the second route establishes that the shortfall is fundamental rather than statistical. We report both.
...
\item[2.] A demonstration (\S{}3--4): a pre-registered diagnostic that killed the obvious explanation of a false-alarm floor and stopped without a fix; then a pre-registered corroboration rule measured on a paired full-population rerun, accepted on a 6.90 $\rightarrow$ 18.19$\times$ raw-floor separation, and stopped again.
...
\item[4.] The composition of the two (\S{}5.2), and its independent confirmation (\S{}5.2.2): applying the first stress test's measured correction to the demonstration's headline puts the shipped detector below its own gate at 8.83$\times$, and a second registration measured on every in-scope object in the archive reaches the same verdict by a direct route at 5.36$\times$, with the shortfall surviving at point estimates on both sides. We report both rather than omit either.
...
Nothing in \S{}5 was repaired. No estimator was swapped, no margin relaxed, no boundary moved, no detector changed, and the 30\textdegree{} rule remains live while the evidence against its stated justification sits in the repository. The reweighted floor is reported at 2.06$\times$ against a 2$\times$ threshold that a registration written three percent looser would have passed, and the raw ratio is published so any reader may apply their own. The composed separation of 8.83$\times$ is reported although the number the apparatus was built to advertise is 18.19$\times$. Phase 3's registered FAIL is reported together with the observation that its own registration predicted failure for the wrong reason, which \S{}5.2.2 states before a reviewer has to.
...
The shape of that change is in Figure~\ref{fig:corroboration}.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/corroboration-slopegraph.pdf}
\caption{The corroboration rule lowered the floor without lowering the capability. Left: passive and payload flag rates on the paired full-population rerun, before and after the rule, on a log scale so that equal proportional falls are equal slopes. Right: the bound separation those two rates produce, against the unchanged requirement. The passive rate falls by roughly two thirds and the payload rate by a few per cent, which is why the separation crosses the requirement from below rather than staying where it was.}
...
The whole verdict is one picture, and Figure~\ref{fig:gate} is it.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/gate-separations.pdf}
\caption{Our own audit shuts our own gate, by two independent routes. Bound separation against the unchanged requirement, drawn as the heavy vertical rule: the raw passive floor with corroboration active clears it, the covariate-matched composite on the one-in-five sample does not, and the full-archive reweighted measurement does not. The whisker on the composite is the registered sensitivity spread, which straddles the requirement rather than settling it. The marked tick below the axis is the point-estimate ceiling, both rates read with no uncertainty admitted anywhere, and it still falls short of the bar: the shortfall is a property of the two populations and not of the sample size.}
\label{fig:gate}
\end{figure}
...
The grid in Figure~\ref{fig:grid} shows where that exposure actually sits.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/covariate-grid.pdf}
\caption{Where the payloads are, the passive objects are not. Passive usable intervals per payload usable interval over the registered perigee and inclination grid, with the eccentricity and cadence factors summed out. Cells outlined in bold carry at least one per cent of all payload exposure, and almost every one of them sits below parity; the cells with passive exposure to spare carry almost no payload exposure at all. That is the mismatch the reweighting has to bridge, and it is why a single pooled floor is the wrong object to gate on.}
\label{fig:grid}
\end{figure}
...
The ordering is drawn in Figure~\ref{fig:timeline} rather than left to a reader to reconstruct from hashes.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/registration-timeline.pdf}
\caption{The registration ordering, drawn from the repository's own clock rather than left to a reader to reconstruct from hashes. Each lane is one experiment: the registration that governs it is committed before its analysis code, and the analysis code before any result. The dashed rule marks the moment the external anchor manifest was generated, after which every registration document is fixed byte for byte. The separation plan that fixes the requirement was committed nine days before the left edge of this window and is therefore off it.}
\label{fig:timeline}
\end{figure}
