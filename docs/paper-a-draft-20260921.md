...
*Disposal-compliance monitoring.* Continuous detection of graveyard raises and of keeping abandonment complements annual outcome reporting rather than replacing it; the census here found 113 distinct payload retirement candidates once a publication-catalogue filter was removed, against 22 in the filtered artifact, with 80% external agreement among assessable cases. The limiter is this paper's own negative result: north-south cessation is not a retirement proxy on its own, so monitoring built on cessation alone would misclassify satellites that are still operating in an inclined orbit (§3.4). The detection threshold is also the bare constant term of the IADC floor rather than the full criterion (§3.8), so a crossing is a detection, not a finding of compliance.
...
The figure below sets out the two steps separately, so that neither is read as the other.

![The census correction is a filter being removed, not data being added. Distinct payloads carrying a detected graveyard-raise flag: in the published artifact, after the publication-catalogue filter is removed from the same archive, and after the pre-registered launch-acquisition screen. The first two columns are like for like; the third is screened, and the hatched step is the payload objects that screen removes rather than the flags it excludes.](latex/paper-a/figs/census-waterfall.pdf)

<!-- figure: docs/latex/paper-a/figs/census-waterfall.pdf, drawn by tools/make_figures.py from docs/eol-policy-relaxation-20260920-receipt.json distinctRetireeCandidates and docs/eol-three-arm-20260920-receipt.json raiseFlow.payloadObjects, raiseFlow.contaminatedFlags and arms.arm1.candidates; derivation: the screen step is the difference of the two published payload-object counts, since the receipt counts that screen in flags -->
...
The figure below puts both arms and the failure marginals in one view.

![Recall, not the supply of retirements, is what stopped the test. Both arms flow from candidate endpoints to usable trajectories to complete matched pairs, and both reach zero against a registered feasibility floor. Below, the arm-one eligibility failures drawn against the full candidate bar: these are overlapping marginal counts and do not sum, and nearly every candidate fails for want of an estimable cadence in one window or the other.](latex/paper-a/figs/recall-wall.pdf)

<!-- figure: docs/latex/paper-a/figs/recall-wall.pdf, drawn by tools/make_figures.py from docs/eol-three-arm-20260920-receipt.json arms.arm1 and arms.arm2: candidates, usable, effects.R_interval.n and eligibilityFailures, with the feasibility floor from docs/eol-policy-relaxation-20260920-receipt.json requiredRetirees -->
...
The distribution behind that median is in the figure below.

![The median is not the story; the distribution is. One point per anchored satellite, ranked, showing the share of the routine north-south keeping budget that detected delta-v accounts for, under both pricings. The dashed rule is the median and the heavy rule is a tenth of the budget. The detector recovers a few per cent of the budget on most of the cohort and essentially none of it on several, which is the recall gap the registered test of this paper ran into.](latex/paper-a/figs/odometer-recall.pdf)

<!-- figure: docs/latex/paper-a/figs/odometer-recall.pdf, drawn by tools/make_figures.py from docs/fuel-odometer-20260920.jsonl sanity.flag, which selects the anchored cohort, and docs/repricing-20260921.jsonl perigee.explainedFractionOfFolkloreBudget and circular.explainedFractionOfFolkloreBudget -->
